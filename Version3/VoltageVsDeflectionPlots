...
Grating1_proFilm(2)  = Grating1_proFilm(2).*1000;
Grating2_proFilm     = proFilmData(:, j:j+1); j =j+2;
Grating2_proFilm(2)  = Grating2_proFilm(2).*1000;
Membrane1_proFilm    = proFilmData(:, j:j+1); j =j+2;
Membrane1_proFilm(2) = Membrane1_proFilm(2).*1000;
KMSF1_proFilm        = proFilmData(:, j:j+1); j =j+2;
KMSF1_proFilm(2)     = KMSF1_proFilm(2).*1000;

%-----------------------------------------------------------------
%Plot data
figure(1); 
subplot(331); plot(Epoxy1(:,1), Epoxy1(:,2)); hold on;
ylabel('Deflection (nm)'); xlabel('Voltage [V]'); hold off;
subplot(332); plot(Epoxy1Test2(:,1), Epoxy1Test2(:,2)); hold on;
ylabel('Deflection (nm)'); xlabel('Voltage [V]'); hold off;
subplot(333); plot(Grating1(:,1), Grating1(:,2)); hold on; 
ylabel('Deflection (nm)'); xlabel('Voltage [V]'); hold off;
subplot(334); plot(KMSF1(:,1), KMSF1(:,2)); hold on; 
ylabel('Deflection (nm)'); xlabel('Voltage [V]'); hold off;
subplot(335); plot(KMSF2(:,1), KMSF2(:,2)); hold on; 
ylabel('Deflection (nm)'); xlabel('Voltage [V]'); hold off;
